              ...
              double *result, double *abserr_round, double *abserr_trunc)
{
        ...
        double e3 = (real_abs(fp1) + real_abs(fm1)) * CML_DBL_EPSILON;
        double e5 = 2.0 * (real_abs(fph) + real_abs(fmh)) * CML_DBL_EPSILON + e3;

        /* The next term is due to finite precision in x+h = O (eps * x) */

        double dy = CML_MAX(real_abs(r3 / h), real_abs(r5 / h)) *(real_abs(x) / h) * CML_DBL_EPSILON;
...
deriv_central(const function_t *f, double x, double h,
              double *result, double *abserr)
{
        ...
        if (round < trunc && (round > 0 && trunc > 0))
        {
                ...
                double h_opt = h * real_pow(round / (2.0 * trunc), 1.0 / 3.0);
...
static void
forward_deriv(const function_t *f, double x, double h,
              double *result, double *abserr_round, double *abserr_trunc)
{
        ...
        double e4 = 2 * 20.67 * (real_abs(f4) + real_abs(f3) + real_abs(f2) + real_abs(f1)) * CML_DBL_EPSILON;

        /* The next term is due to finite precision in x+h = O (eps * x) */

        double dy = CML_MAX (real_abs(r2 / h), real_abs(r4 / h)) * real_abs(x / h) * CML_DBL_EPSILON;
...
__CML_EXTERN_INLINE int
deriv_forward(const function_t *f, double x, double h,
              double *result, double *abserr)
{
        double r_0, round, trunc, error;
        ...
        if (round < trunc && (round > 0 && trunc > 0))
        {
                ...
                double h_opt = h * real_pow(round / (trunc), 1.0 / 2.0);
...
__CML_EXTERN_INLINE int
deriv_backward(const function_t *f, double x, double h,
               double *result, double *abserr)
{
